function attRes = AttEst_epochs(tseq, varData, seqTimes, DEqns, DEpars, t0, Tperiod, attEstParams, dScaleThresh, ...
                         tScaleThresh, numExt, numInt, varnames, actsIxMap, inputsIx, varBounds, figHandle, verboseTog)
% Attractor Estimation procedure for a single local variable
% Version 1.42, May 2004
% (c) Robert Clewley, Center for Biodynamics, Boston University
%
% Notes about parameters (including those for main auxiliary functions,
%                         CompressEst(), NeighbHdEst() ):
% focusVarIx   defines a variable of focus (absolute index in set 'varnames')
...
% dScaleThresh is the `sigma` dominance scale threshold used to generate the functional network
% Vpert        defines the +/- distance from V(t0) used as an initial target domain
% tScaleThresh defines the scale threshold for the fast time-scale of fast V-dependent variables
% delta        is the time tolerance at chosen critical epoch changeover,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% DEVELOPMENT NOTES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Version 1.43, June 2005
%   Slight alteration to way in with PrepareFocVars avoids focused variable
%   in loop.
%
% Version 1.42, May 2004
%   Changed name of this function to AttEst_epochs, because of new version AttEst_regimes,
%    which works on a set of reduced regimes rather than epochs alone. This `epochs-only`
%    version may be retired in the next main upgrade...
%
% Version 1.41, April 2004
%   Changed time scale threshold from being in attEstParams, to being a separate param.
%
% Version 1.40, April 2004
%   Moved setting of actives switches in DEqns to its own function.
%   Replaced use of the GLOBAL focFastScaleThresh by argument attEstParams.fastScaleThresh
%   Ideas from SLACKFLAG implementation lead to new and conceptually simpler version of
%    CompareActs(), without SLACKFLAG or ignoreActs_flag (although the latter's code is
%    still present.
%   Renamed all occurrences of `obsIx` to `focusVarIx` (which were both present and identical).
%
% Version 1.3, March 2004
%   SLACKFLAG added, for the SLACK option in generating restricted attractor basin
%    estimates. This option is still in development but currently replaces the
%    ignoreActs_flag option.
%
% Version 1.2, February 2004
%   Overhauled focus set processing to measure relative time scales before producing
%    estimating variable values.
%   Added computation of concatenated Phi map over entire time interval [t0,t0+Tperiod]
%    with displayed results.
%
% Version 1.0, January 2004
%   Transition sequence focus option is now a list, so that all focusVarIx-dependent
%    variables are included, not just originally focused variable `focusVarIx`. This helps
%    AttEst to choose appropriate epochs to analyze for the "simpler" variables such
%    as gating variables, which don't themselves have changes in *their* Actives set
%    but need to be analyzed at times relevant to their dependents (e.g. referring
%    back to the voltage variable from the m gating variable).
%   From this, AttEst now must check changes to Actives sets for all focusVarIx-dependent
%    variables as well as focusVarIx itself, over selected time window.
%   Additionally, for sparse time intervals, AttEst adds artificial epoch partitions
%    to avoid errors caused by poor low-resolution temporal partitioning of orbit.
%    This has been called 'bolstering' the sample set of times for attractor estimations.
%    These have a minimum interval which depends on the approximate rate of change of
%    the focused variable. (These partitions are also coloured differently in the
%    AttEst plot, using magenta rather than red lines.) See documentation for details.
%   As a result, AttEst gives correct results for all neural variables in the demos!
%   Added 'drawnow' figure refresh commands in main loop.
%   Added user-interrupt 'escape' key command (checked once for each epoch).
%   Main search algorithm has been completely overhauled to improve accuracy and speed.
%    It is now based on a binary search of a V interval, with the twist that the
%    interval is not necessarily 'sorted', i.e. it can pass at both (initial) ends
%    but fail somewhere in the middle. Thus there is an initial uniformly-spaced low
%    resolution linear search to find the largest 'sorted' interval (with no
%    pass-fail-pass property).
%   Additionally, bounds and precision checking during the search has been added
%    (fixing problems with very 'stiff' dynamics, e.g. for `m` gating variable)
%
% Version 0.9, December 2003.
%   This version generalises the original test version of this function, so now it
%    can be applied to any external variable in the system, even though currently
%    it only gives correct results for voltage variables.
%  Added focus option to transition sequences to allow less cluttered, more relevant
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global figHand maxVresFrac diagnosticOn
GlobalConstants
...
ignoreActs_flag = false; % THIS OPTION IS IN DEVELOPMENT. IT DOESN'T YET WORK!
...
epochs = {}; % focused on the variable chosen
attRes = {success, epochs, 0, 0, 0}; % default result (error status)
...
disp('AttEst:  This is the `epochs-only` version of AttEst')
...
if ~isempty(TSpads)
    beep
    disp('AttEst:  Invalid Transition Sequence: no pads allowed.')
    return
end
...
if t0 < 0
    beep
    disp('AttEst:  t0 must be greater or equal to zero.')
    return
end
if Tperiod <= 0
    beep
    disp('AttEst:  Tperiod must be greater than zero.')
    return
end
...
if ~isempty(TSfocusSet)
    ...
    disp('          most effective for focused transition sequences')
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                    EPOCH INITIALIZATION (including BOLSTERING)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

disp('AttEst:  Initializing epoch data ... ')

% Use shorter names for these variables, for convenience
delta  = attEstParams.delta; % currently unused
dVmax  = attEstParams.dVmax;
Vinres = attEstParams.Vinres;
Vpert  = attEstParams.Vpert;
% NOTE: searchRes and lowResMultiple must be integers
searchRes      = attEstParams.searchRes; % search resolution (also lowest bolster epoch interval size (highest resolution))
lowResMultiple = attEstParams.lowResMultiple; % multiplies searchRes
derivThresh    = attEstParams.derivThresh; % used to generate derivative-estimate threshold to switch on high resolution bolster epochs
...
numEpochs = TSlength; % includes repeated final entry in the case of cycles
if numEpochs < 1
    disp(' ')
    disp('AttEst:  Fewer than one epoch present in passed transition sequence.')
    disp('          Cannot proceed.')
    return
end
...
% Diagnositc counters for bolstered epochs
numBlong = 0;
numBshort = 0;
numBmedium = 0;

numEpsTot = 0;
for pos = 1:numEpochs % numEpochs is not total # of epochs used in analysis, because we might add some bolster epochs
    if verboseTog
        fprintf('          Epoch %3i',pos)
    end
    ts_temp = tseq{TS_TSEQ}{pos};
    % Note: [tlo, thi] are time intervals *relative* to t0. For absolute times, add t0
    tlo = ts_temp{2};
    if pos < numEpochs
        thi = tseq{TS_TSEQ}{pos+1}{2};
    else
        thi = Tperiod;
        if thi < tlo
            beep
            disp(['          Specified period smaller than epoch ' num2str(pos) '`s interval'])
    ...
    %%%%%% Work out which are actives are present for the whole epoch (including bolster epochs)
    allActives = ts_temp{1}; % one set for every external variable (but only those in TSfocusSet are nonempty)
    acts = allActives{focusVarIx}; % these are indices relative to the set of inputs to focusVarIx
    actNames = {};
    if verboseTog
        fprintf('            Actives for primary focused variable:  ')
    end
    for entry = 1:length( acts )
        newName = varnames{actsIxMap{focusVarIx}(acts(entry))};
        if verboseTog
            fprintf('%s ',newName);
        end
    ...
    %%%%%% Add regular epoch to epoch list, but ...
    %   Flag the need for additional bolster epochs in this [tlo,thi],
    %       depending on maximum of derivatives of all focused variables!
    %   Note: Final epoch is only there for reference (it's time interval *starts*
    %       at the end of the user-selected time interval in StepNet), and so we
    %       don't need to bolster this one. We only need to record it for the sake
    %       of knowing the Actives set during this epoch, during NeighbHdEst()
    epTlo = tlo; % current lower epoch time
    [isp epTloPos] = ismember(1,seqTimes <= epTlo+t0); % was FindPosInT(epTlo,t0,seqTimes);
    tloPos = epTloPos; % true at this initial point
    ...
    if pos == numEpochs % final epoch is special: see above note
        numEpsTot = numEpsTot + 1;
        epochs = {epochs{:} {numEpsTot; [tlo, thi]; [tloPos, thiPos]; allActives; actNames; dummyVInterval; true} };
        % force origFlag = true so that red vertical line appears at end of attractor estimate diagram.
        continue
    end
    addEpochs = true; % initial value (adding bolster epochs to the original epoch interval)
    bolstered = false; % flag to indicate whether bolster epochs were added
    searchMarkPos = -1; % dummy value indicating "currently not in use"
    tsearch_old = epTlo; % initial value (start at tlo, and then whatever epTlo is set subsequently)
    tsearchPos_old = epTloPos; % initial value
    origIntPos = [tloPos, thiPos]; % original time interval for epoch, in terms of positions
    varData_old = varData(tsearchPos_old,:);
    dVarRel = zeros(1,numVarsFocused);
    while addEpochs
        origFlag = false;
        tsearch_new    = tsearch_old + searchRes*dt;
        tsearchPos_new = tsearchPos_old + searchRes;
        if tsearchPos_new >= thiPos % then end of an original epoch (not considered to be adding a bolster epoch here)
            if ismember( epTloPos, origIntPos )
                origFlag = true; % part of an original (non-bolster) epoch
            end
            if epTloPos < thiPos
                numEpsTot = numEpsTot + 1;
                epochs = {epochs{:} {numEpsTot; [epTlo, thi]; [epTloPos, thiPos]; allActives; actNames; ...
                            dummyVInterval; origFlag} };
            else
                if verboseTog | diagnosticOn
                    disp('            Warning while bolstering epochs: `consecutive` time positions were in fact not!')
                    disp('              (skipping)')
                end
            end
            addEpochs = false; % no more time left in the original epoch interval [tlo, thi]
            continue
        end
        if tsearchPos_new == searchMarkPos % reached a lo-res marker set earlier
            ...
            if epTloPos < tsearchPos_new
                numEpsTot = numEpsTot + 1;
                numBlong = numBlong + 1;
                epochs = {epochs{:} {numEpsTot; [epTlo, tsearch_new]; [epTloPos, tsearchPos_new]; allActives; ...
                            actNames; dummyVInterval; origFlag}};
            else
                if verboseTog | diagnosticOn
                    disp('            Warning while bolstering epochs: `consecutive` time positions were in fact not!')
                    disp('              (skipping)')
        ...
        if max(dVarRel) >=  searchRes*dt*derivThresh
            % allocate high resolution --> epoch boundary here at tsearch position
            % close up the present epoch if it wasn't a high res bolster epoch
            nextEpTlo    = tsearch_old; % default value
            nextEpTloPos = tsearchPos_old; % default value
            if epTloPos ~= tsearchPos_old
                if ismember( epTloPos, origIntPos )
                    origFlag = true; % part of an original (non-bolster) epoch
                end
                if epTloPos < tsearchPos_old
                    ...
                    epochs = {epochs{:} {numEpsTot; [epTlo, tsearch_old]; [epTloPos, tsearchPos_old]; ...
                                allActives; actNames; dummyVInterval; origFlag}};
                else
                    if verboseTog | diagnosticOn
                        disp('            Warning while bolstering epochs: `consecutive` time positions were in fact not!')
                        disp('              (skipping)')
                    ...
                    nextEpTlo    = epTlo;
                    nextEpTloPos = epTloPos;
                end
            end
            origFlag = false; % default value here (may be changed below)
            % add high res epoch
            if ismember( nextEpTloPos, origIntPos )
                origFlag = true; % part of an original (non-bolster) epoch
            end
            if nextEpTloPos < tsearchPos_new
                numEpsTot = numEpsTot + 1;
                numBshort = numBshort + 1;
                epochs = {epochs{:} {numEpsTot; [nextEpTlo, tsearch_new]; [nextEpTloPos, tsearchPos_new]; ...
                            allActives; actNames; dummyVInterval; origFlag}};
            else
                if verboseTog | diagnosticOn
                    disp('            Warning while bolstering epochs: `consecutive` time positions were in fact not!')
            ...
            epTlo = tsearch_new; % for next iteration
            epTloPos = tsearchPos_new;
            searchMarkPos = -1; % reset this, since we had to add a high-res epoch in the meantime
            bolstered = true;
        else
            % if it hasn't already been set, then set searchMarkPos ...
            % allocating low resolution, i.e. bolster epoch end at tsearchPos_old + lowResMultiple * searchRes,
            % pending search through remainder of the [tlo,thi] interval
            if searchMarkPos == -1
                searchMarkPos = tsearchPos_old + lowResMultiple * searchRes;
            end
    ...
    if verboseTog & bolstered
            fprintf('            Bolster epochs were added (%i short, %i medium, %i long)\n', numBshort, numBmedium, numBlong)
    end
end
...
numEps = numEpsTot-1; % we only use the last (unbolstered, i.e. original) epoch as a reference for Actives at end time
...
% PhiMaps will hold coefficient and offset for affine linear Phi map of each epoch *forward* in time
PhiMaps = zeros(numEps,2);
...
% Step through epochs backwards from the final one (remember, numEps = numEpsTot - 1)
for pos = 1:numEps % can multiply by an integer to go around a cycle more times, if a cycle is being studied
%     if isCycle
%         prevEpochPos = mod(-pos-1,numEps)+1;
%     else
%         prevEpochPos = numEps-pos;
%     end
%     if isCycle
%         epochPos = 1+mod(prevEpochPos,numEps);
%     else
%         epochPos = prevEpochPos + 1;
%     end
    epochPos = numEps-pos+1;
    epoch = epochs{epochPos}; % searches backwards!
    epochAbsId = epoch{1}; % absolute ID number of epoch (according to order established above)
    nextEpochPos = 1 + epochPos;
    nextEpochAllActs = epochs{nextEpochPos}{4}; % the one forward in time -- wraps around, assuming periodic orbit! 
    epochAllActs = epoch{4}; % active sets for all variables
    epochActNames = epoch{5}; % for screen display use (primary focused variable only)
    tlo = epoch{2}(1);
    thi = epoch{2}(2);
    poslo = epoch{3}(1);
    poshi = epoch{3}(2);

    % check if user is trying to interrupt AttEst (or, in future, change message verbosity)
    drawnow
    ...
    %%% TEMPORARY %%%
    if epochPos == numEps % then this is last epoch in sequence = first to analyze, so set the target here
        lastVval = varData(poshi,focusVarIx); % and use this value at end of function for error cf. V_Phi_mapped
        initVinterval = [lastVval-Vpert,lastVval+Vpert]; % initial value only
        Vend = initVinterval;
    ...
    DEqns_comp = SetActivesSwitch(DEqns_comp, TSfocusSet, deqnIxMap, numExt, actsIxMap, epochAllActs);
    ...
    if verboseTog
        ...
        fprintf('AttEst:  Beginning analysis of epoch %i\n',epochAbsId);
        if pos > numEps % replace this later with explicitly known cycle number
            disp( '          ... another time around a cycle')
        end
        fprintf('          Time interval is [%.2f, %.2f] relative, [%.2f, %.2f] absolute\n',tlo,thi,tlo+t0,thi+t0);
        fprintf('            %s is active\n',epochActNames{:});
        if epochPos == numEps % then first epoch analyzed
...
%         vData, tData, ignoreActs_flag, verboseTog);
    ...
    PhiMaps(epochAbsId,:) = [compressResult(1) compressResult(2)];

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%% Neighbourhood Estimate at t1 %%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % (this is a form of "shooting")

    if ~isempty(compressResult) % then proceed (arbitrary check for error status from CompressEst)
        % Vt1 = V(tlo) from known orbit
        % Vend = V(thi) target interval
        Vt1 = vData(1,focusVarIx);
        ...
        neighbResult = NeighbHdEst(TSfocusSet, varnames, deqnIxMap, allDEixMap, DEqns_comp, DEpars, trivialCandActs, ...
            VdepInfo, varBounds, tScaleThresh, tlo, thi, vData, dt, compressResult, numExt, ...
            Vt1, Vend, Vinres, dVmax, dScaleThresh, epochAllActs, nextEpochAllActs, absGamIxMap, ...
            actsIxMap, firstInterval_flag, ignoreActs_flag, verboseTog);
        if isempty(neighbResult{1})
            disp('AttEst:  Fatal error occurred. Returning.')
            return
        end
        ...
        epochs{epochPos}{6} = Vinterval;
        Vend = Vinterval; % for use with the next epoch
    else
        beep
        fprintf('          Compression estimate failed for epoch %i\n',epochAbsId)
        return
    end % if
end % for pos

% set this interval in dummy final epoch (numEpsTot, not numEps) to be the initial target interval (for plotting purposes)
epochs{numEpsTot}{6} = initVinterval;
...
for epIx = 1:numEps % this time go forwards in time
...
attRes = {success, epochs, numEps, numEpsTot, dt};
return
...
%%% Calculate q.s.f.p. of focusVarIx at t1
...
if steps > lenVarData
    if verboseTog | diagnosticOn
        fprintf('CompressEst:  Debugging info: steps > lenVarData by %i\n',steps-lenVarData);
    end
...
for tstep=steps:-1:1 % go from end backwards (more efficient re-use of partial sums)
    ...
    % Add to the integral that measures compression over the epoch
...
% Calculate q.s.f.p. of focusVarIx at t2
...
% offset (second return argument) incorporates epoch correction x
...
function result = NeighbHdEst(focusSet, varnames, deqnIxMap, allDEixMap, DEqns, DEpars, trivialCandActs, ...
            ...
            Vknown_t1, Vend, Vinres, dVmax, dScaleThresh, acts_t1, acts_t2, absGamIxMap, ...
            absActsIxMap, firstInterval_flag, ignoreActs_flag, verboseTog)
% Function to calculate estimate of inverse Phi map's domain (using reduced model as determined by DSSRT)
%  over one epoch. (Compare with numerically obtained neighbourhood by regular shooting.)
...
global DE_NAMEi DE_GAMMA1i DE_GAMMA2i DE_ACTSWi DE_ISTAUFFILEi DE_TAURECIPi DE_GAMVARNAMEi ...
    DE_GAMVARPOWi DE_ISTGTFFILEi DE_TARGETi DE_INTVARNAMEi DE_INTVARPOWi maxVresFrac diagnosticOn
global figHand LARGEBOUND SMALLBOUND
% global focFastScaleThresh

result = {[],[]}; % default value (error status)
...
V0t1         = compressResult(3);
V0t2         = compressResult(4);
% Thus, Phi map over the epoch is Vt2 = Vt1 * expGhat + offset
...
if expGhat <= SMALLBOUND % DSSRT precision level
    ...
    if verboseTog | diagnosticOn
        disp('          No calculations done - compression ratio too high and orbit is very flat in this epoch')
    end
    if firstInterval_flag % make sure we don't return an out-of-bounds target Vend (this code copied from below)
        ...
        if sum(bdTest) ~= 0
            if verboseTog | diagnosticOn
                disp('          Initial target V interval Vend outside variable bounds in first epoch analyzed ...')
            end
            if bdTest(1)
    ...
    result = { [max(varBounds(primaryFocIx,1),Vknown_t1-SMALLBOUND), min(varBounds(primaryFocIx,2),Vknown_t1+SMALLBOUND)], Vend };
    return
end

% Check legitimacy of initial Vend using binary search, if this is first epoch of sequence
infSwitchOn = true;
...
%             fastFocVars = focusedInfo.fastFocVars;
            for focusSetIx = 1:numFocused
                ...
                %% Prepare estimated varDataLine and find fast V-dependent variables at start of epoch
...
%                 if ~isempty(setxor( intersect(focusedInfo.fastFocVars,abs_acts_t2) ,intersect(focusedInfo_update.fastFocVars,abs_acts_t2)))
                ...
                match = CompareActs(absE_b_t2, abs_acts_t2, VdepInfo{focusSetIx}, pestInfo.fastRelVars);
            ...
            if sum(success) == numFocused % then all passed for this Vend
                ...
                if infSwitchOn && abs(Vend(VendIx) - varData(lenVarData,primaryFocIx)) < Vinres*1.1 % need *1.1 because VendCpoint could be set one Vinres from Vknown_t1, and so this will never quite be true (would if <=) after asymptoting through repeated halving in the failure case -- eventually causing VendCpoint == VendEpoint error in failure case
                    ...
                    if VendEpoint(VendIx) == VendCpoint(VendIx)
                        disp('             In first epoch analyzed (checks at t2):')
...
firstVend = Vend; % a record of what Vend is, in case it was changed for the first epoch of an attractor estimate above
...
if Vend_at_t1(1) > bounds_t1(2) | Vend_at_t1(2) < bounds_t1(1)
    disp(   '          Fatal error! Something messed up at start of epoch -- Vend mapped to t1 no longer contains known V(t1)')
...
%  The points of failure in each direction will be the maxium search interval for the binary search
...
%         if ~isempty(setxor( intersect(focusedInfo.fastFocVars,abs_acts_t1),intersect(focusedInfo_update.fastFocVars,abs_acts_t1)))
        ...
        if match(1)
            if ignoreActs_flag
                currFast = intersect(pestInfo.fastRelVars,abs_acts_t1);
                if ~isempty(currFast)
                    match = CompareActs(absE_b_t1, abs_acts_t1, VdepInfo{focusSetIx}, ...
                        setdiff(pestInfo.fastRelVars, currFast));
                else
                    match = CompareActs(absE_b_t1, abs_acts_t1, VdepInfo{focusSetIx}, pestInfo.fastRelVars);
                end
            else
                match = CompareActs(absE_b_t1, abs_acts_t1, VdepInfo{focusSetIx}, pestInfo.fastRelVars);
            end
...
VendCpoint = [ varData(lenVarData,primaryFocIx), varData(lenVarData,primaryFocIx) ]; % default centre point position of V interval
VendEpoint = Vend; % starting end points of the V interval
if diagnosticOn
...
for stage=2:3
    if stagePositions(stage) > 0
        ...
        if stageOneResult(stage-1) % the inner layer point passed
            ...
            % leave centre point on orbit
            VendEpoint(stage-1) = initialCheckVals(stage-1)*expGhat + offset; % set centre point to a known failing point
            Vend(stage-1) = ( VendEpoint(stage-1) + VendCpoint(stage-1) ) / 2; % test this val next, in binary search
            changedVend(stage-1) = changedVend(stage-1)+1;
            if diagnosticOn
    ...
    while failBinSearch(VendIx)
        ...
        if diagnosticOn
            ...
            if get(figHand,'CurrentCharacter') == 27
                disp(' ')
                beep
                disp('AttEst:  Within binary search: attractor estimate operation cancelled by user!')
                return
            end
        ...
        focusedInfo = PrepareFocVars( tScaleThresh, varData(1,:), primaryFocIx, Vend_at_t1(VendIx), focusSet, ...
            DEqns, allDEixMap, infSwitchOn );
        varDataLine = focusedInfo.varData;
        for focusSetIx = doFocusSet
            absFocusedIx = focusSet(focusSetIx);
            if absFocusedIx > numExt % then it's an internal variable so don't do this loop
                continue
...
if length(VdepList) > 0
    fastVars = ScaleThreshSort(varSpeeds, fastScaleThresh);
    absFastVars = VdepList(fastVars); % array for absolute variable indices of fastVars relative to focused variable
	for g1t = 1:length(Gamma1) % will have to do this for Gamma2 too later on
        [ pres pix ] = ismember( Gamma1{g1t}{DE_INTVARNAMEi}, absFastVars );
        if pres
            absFastVars(pix) = Gamma1{g1t}{DE_GAMVARNAMEi};
        end
	end
...
% We only check for V-dependent variables (because of short-term independence of q.s.f.p. on other variables)
...
%  on which ones were active in the previous epoch. So far, we're picking out the fastest
%  V-dep variables to focus on the most...
%
function result = CompareActs(actsPert, actsRef, VdepIxList, VdepFastIxList)
% NOTE - order of these actives sets as parameters is crucial!
...
lenVdepFIL = length(VdepFastIxList);
lenApert = length(actsPert);
lenAref = length(actsRef);
actsP = [];
actsR = [];

%% Form reduced Actives sets, with only V-dependent variables included (in original order)
% Don't do this using `intersect` so that we guarantee the preserved order of the entries
for aix = 1:lenApert
...
for aix = 1:lenAref
    if ismember(actsRef(aix),VdepFastIxList)
        actsR = [actsR actsRef(aix)]; % keep most active at front of list
    end
end

if isempty(setxor(actsR,actsP))
    result(1) = true; % match
    return
else
    result(2) = 1; % mismatch
...
% In need of implementing this function to get true 'regime' estimation
function newEpochs = CleanEpochs(epochs, timeRes)
% Compacts epochs with transient in/out actives & multiple changes to Acts over short time periods
% ("short times" <= rel. to the dynamics -- e.g. not during an action potential)
newEpochs = {};
return
